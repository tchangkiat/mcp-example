...
server.tool("get-singapore-weather", "Get weather for Singapore", async () => {
  // Get weather data
  const weatherData =
    await makeWeatherDataRequest<WeatherDataResponse>(API_BASE);
  if (!weatherData) {
    return {
      content: [
        {
  ...
  const weatherText =
    ...
    weatherData.data.readings[0].data[0].value +
    "°C";
