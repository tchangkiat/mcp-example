...
class MCPClient {
  ...
  async connectToServer(serverScriptPath: string) {
    try {
      ...
      this.tools = toolsResult.tools.map((tool) => {
        return new Tool(tool.name, tool.description, tool.inputSchema);
      });
      console.log(
  ...
  async invoke(prompt: string) {
    const response = await this.bedrockClient.invokeAnthropicModel(
      prompt,
      this.tools,
    );
    ...
    for (const content of response.content) {
      if (content.type === "text") {
        ...
        const toolResultContent = toolResults.content as {
          type: string;
          text: string;
        }[];
        finalText.push(
        ...
        const response2 = await this.bedrockClient.invokeAnthropicModel(
